...
class HunyuanVideoModel(BaseModel):
    ...
    def additional_conditionals(
        ...
        """Adds any extra conditionals to the model, such as a cropped attention mask.

        Args:
            batch (dict): Contains "input_mask" which may be cropped based on the
                prompt_template["crop_start"] value.
            timesteps (torch.Tensor): Denoising timesteps for which conditionals are applied.

        Returns:
            tuple:
                - dict: A dictionary containing "encoder_attention_mask" if applicable.
        ...
        prompt_attention_mask = batch["input_mask"]
        crop_start = self.prompt_template["crop_start"]
        if crop_start is not None and crop_start > 0:
